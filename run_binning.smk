...
###################################################
# Make the plots from the stratification analysis #
###################################################
rule table_to_plot:
  input:
    stratified_counts_table = "stratified_counts_table/{sample}/{ref_name}/counts_table.txt",
    table_to_plot = config["table_to_plot"]
  output:
    total_plot = "stratified_counts_plot/{sample}/{ref_name}/total_count.png",
    fp_plot = "stratified_counts_plot/{sample}/{ref_name}/fp_ratio.png"
  threads:
    1
  shell:
    """
    ...
    outdir=$(dirname {output.total_plot})
    mkdir -p $outdir
